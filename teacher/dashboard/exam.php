   ...
   $sql=mysqli_query($con,"SELECT * FROM `sms_s-notification` WHERE `active`='1' AND `receiver`='".$_SESSION['name']."'");
    ...
    if(isset($_POST['submit1'])){
        ...
        for($i=1;$i<=$tot;$i++){
            ...
            $qnq=mysqli_query($con,"INSERT INTO `sms_questions`(`qid`, `question`, `q_no`) VALUES ('".$id."','".$qns."','".$i."')");
            if($qnq){
                $fetc=mysqli_query($con,"select qn_id from sms_questions where qid='".$id."' and q_no='".$i."'");
                $f=mysqli_fetch_assoc($fetc);
                
                $qa=mysqli_query($con,"INSERT INTO `sms_options`(`qn_id`, `value`) VALUES ('".$f['qn_id']."','".$a."')");
                $qb=mysqli_query($con,"INSERT INTO `sms_options`(`qn_id`, `value`) VALUES ('".$f['qn_id']."','".$b."')");
                $qc=mysqli_query($con,"INSERT INTO `sms_options`(`qn_id`, `value`) VALUES ('".$f['qn_id']."','".$c."')");
                $qd=mysqli_query($con,"INSERT INTO `sms_options`(`qn_id`, `value`) VALUES ('".$f['qn_id']."','".$d."')");

                switch($correct){
                    case 'a': $fa=mysqli_query($con,"select oid from sms_options where qn_id='".$f['qn_id']."' and value='".$a."'");
                              $fe=mysqli_fetch_assoc($fa);
                              $ans=$fe['oid'];
                              break;
                    case 'b': $fa=mysqli_query($con,"select oid from sms_options where qn_id='".$f['qn_id']."' and value='".$b."'");
                              $fe=mysqli_fetch_assoc($fa);
                              $ans=$fe['oid'];
                              break;
                    case 'c': $fa=mysqli_query($con,"select oid from sms_options where qn_id='".$f['qn_id']."' and value='".$c."'");
                              $fe=mysqli_fetch_assoc($fa);
                              $ans=$fe['oid'];
                              break;
                    case 'd': $fa=mysqli_query($con,"select oid from sms_options where qn_id='".$f['qn_id']."' and value='".$d."'");
                              $fe=mysqli_fetch_assoc($fa);
                              $ans=$fe['oid'];
                              break;
                    default:  $fa=mysqli_query($con,"select oid from sms_options where qn_id='".$f['qn_id']."' and value='".$a."'");
                              $fe=mysqli_fetch_assoc($fa);
                              $ans=$fe['oid'];
                }
                $qans=mysqli_query($con,"INSERT INTO `sms_answers`(`qn_id`, `oid`) VALUES ('".$f['qn_id']."','".$ans."')");
            }else{
                ?>
                <script>
        ...
        ?>
            <script>
                alert("Questions and details uploaded Successfully");
            </script>
        <?php
    }
            ...
            <div class="dashboard-ecommerce">
                <div class="container-fluid dashboard-content ">
                    ...
                    <form method="post" action="#">
                        ...
                        <input type="submit" name="ok" placeholder="Submit">
                    </form>
                    <?php
                     if(isset($_POST['ok'])){
                        $id=$_POST['id'];

                        $cod=mysqli_query($con,"select * from sms_quiz where qid='".$id."'");
                        if(mysqli_num_rows($cod)>0){
                            $fet=mysqli_fetch_assoc($cod);
                            ?><br><br><h3 align="center"><b>Enter Questions And Details</b></h3><?php
                            for($i=1;$i<=$fet['total_q'];$i++){
                                ?>
                                <form method="post" action="#">
                                    <input type="hidden" name="tot" value="<?php echo $fet['total_q']; ?>">
                                    <input type="hidden" name="id" value="<?php echo $id; ?>">
                                <b>Question number&nbsp;<?php echo $i; ?>&nbsp;:</><br></b>
                                <!-- Text input-->
                                <div class="form-group">
                                    <label class="col-md-12 control-label" for="qns<?php echo $i; ?>"></label>  
                                    <div class="col-md-12">
                                        <textarea rows="3" cols="5" name="qns<?php echo $i; ?>" class="form-control" placeholder="Write question number <?php echo $i; ?> here..." required></textarea>  
                                    </div>
                                </div>
                                <!-- Text input-->
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <input name="<?php echo $i; ?>1" placeholder="Enter option a" class="form-control input-md" type="text" required>
                                    </div>
                                </div>
                                <!-- Text input-->
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <input name="<?php echo $i; ?>2" placeholder="Enter option b" class="form-control input-md" type="text" required>
                                    </div>
                                </div>
                                <!-- Text input-->
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <input name="<?php echo $i ?>3" placeholder="Enter option c" class="form-control input-md" type="text" required>
                                    </div>
                                </div>
                                <!-- Text input-->
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <input name="<?php echo $i; ?>4" placeholder="Enter option d" class="form-control input-md" type="text" required>
                                    </div>
                                </div>
                                <br>
                                <b>Correct answer</b>:<br>
                                <div class="col-md-6">
                                    <select name="ans<?php echo $i; ?>" placeholder="Choose correct answer " class="form-control input-md" required>
                            ...
                            ?>
                                <div class="form-group">
                                    ...
                                    <div class="col-md-12"> 
                                        <input  type="submit" style="margin-left:45%" class="btn btn-primary" value="Submit" class="btn btn-primary" name="submit1">
                                    </div>
                                </div>
                            </form>
                            <?php
