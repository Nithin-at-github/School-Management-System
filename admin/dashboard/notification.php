 ...
 if(isset($_SESSION['id'])){
  ...
  if(isset($_POST['submit_n'])){
    ...
    if(isset($_POST['notify'])){
        $name=$_POST['notifications_name'];
        $message=$_POST['message'];

        foreach ($_POST['receiver'] as $key) {
            if($key=='teacher'){
                $teach=mysqli_query($con,"select t_fullname from sms_teachers");
                while ($f=mysqli_fetch_assoc($teach)) {
                    mysqli_query($con,"INSERT INTO `sms_t-notification`(`notifications_name`, `message`, `active`, `receiver`) VALUES ('".$name."','".$message."','1','".$f['t_fullname']."')");
                }
            }
            if($key=='student'){
                $stud=mysqli_query($con,"select s_fullname from sms_students");
                while ($fe=mysqli_fetch_assoc($stud)) {
                    mysqli_query($con,"INSERT INTO `sms_s-notification`(`notifications_name`, `message`, `active`, `receiver`) VALUES ('".$name."','".$message."','1','".$fe['s_fullname']."')");
                }
            }
            if($key=='parent'){
                $par=mysqli_query($con,"select p_name from sms_parents");
                while ($fet=mysqli_fetch_assoc($par)) {
                    mysqli_query($con,"INSERT INTO `sms_p-notification`(`notifications_name`, `message`, `active`, `receiver`) VALUES ('".$name."','".$message."','1','".$fet['p_name']."')");
                }
            }
        ...
        <div class="dashboard-wrapper">
            <div class="dashboard-ecommerce">
                <div class="container-fluid dashboard-content ">
                    ...
                    <!-- ============================================================== -->
                    <div class="container"> 
                        <form class="form-horizontal" id="frm_data" method="post" action="#">
                            <div class="form-group row">
                                <label class="control-label col-md-2" for="notification">Name</label>
                                <div class="col-md-6">
                                    <input type="text" name="notifications_name" id="notifications_name" class="form-control" placeholder="Notification name" required/>
                                </div> 
                            </div>   
                            <div class="form-group row">
                                <label class="control-label col-md-2" for="notification">Message</label>
                                <div class="col-md-6">
                                    <textarea style="resize:none !important;"name="message" id="message" rows="4" cols="10" class='form-control'></textarea>
                                </div> 
                            </div>
                            <div class="form-group row">
                                <label class="control-label col-md-2" for="notification">Receiver</label>
                                <div class="col-md-6" style="font-size: 20px;">
                                    <input type="checkbox" name="receiver[]" value="teacher" required/>&nbsp;Teachers&emsp;
                                    <input type="checkbox" name="receiver[]" value="student" required/>&nbsp;Students&emsp;
                                    <input type="checkbox" name="receiver[]" value="parent" required/>&nbsp;Parents&emsp;
                                </div> 
                            </div>
                            <div class="form-group row">
                                <div class="col-md-10 col-offset-2" style="text-align:center;">
                                    <input type="submit" id="notify" name="notify" class="btn btn-primary" value="NOTIFY"/>
                                </div>
                            </div>   
                        </form>       
                    </div>
