<?php
 ...
 if(isset($_SESSION['id'])){
    ...
    if(isset($_POST['approve'])){
        ...
        mysqli_query($con,"UPDATE `sms_leave` SET `status`='Approved' WHERE `id`='".$id."'");
    }

    if(isset($_POST['decline'])){
        $id=$_POST['decline'];

        mysqli_query($con,"UPDATE `sms_leave` SET `status`='Declined' WHERE `id`='".$id."'");
    }
                      ...
                      <div class="tab-pane fade active show" id="inbox" aria-labelledby="inbox-tab" role="tabpanel">
                        <div>
                            <div class="row p-4 no-gutters align-items-center">
                                <?php
                                  $act=mysqli_query($con,"select * from sms_leave where status='In Review'");
                                  $c=mysqli_num_rows($act);
                                ?>
                                <div class="col-sm-12 col-md-6">
                                    <h3 class="font-light mb-0"><i class="ti-email mr-2"></i><?php echo $c; ?> unreaded Requests</h3>
                                </div>
                                <div>
                                    <form action="#" method="post">
                                        <button type="submit" name="show"><i class=" fas fa-eye">&nbsp;Show all requests</i></button>
                                    </form>
                                </div>
                            </div>
                            <!-- Mail list-->
                            <?php
                                      if(isset($_POST['show'])){
                                        $show=mysqli_query($con,"select * from sms_leave order by id desc");
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                <span class="text-dark"><?php echo $s['reason']; ?></span>
                                                                </a>
                                                                <div id="<?php echo $i; ?>" class="collapse submenu">
                                                                    <pre style="font-family: Franklin Gothic; font-size: 15px;"><?php echo $s['description']; ?><br><br>From :&nbsp;<?php echo $s['d-from']; ?>       To :&nbsp;<?php echo $s['d-to']; ?>
                                                                    </pre>
                                                                    Status :&nbsp;<?php echo $s['status']; ?>
                                                                </div>
                                                            </td>
                                                            <!-- Time -->
                                                            <td class="text-muted"><?php echo $s['date']; ?></td>
                                                        </tr>
                                                            <?php
                                                                $i++;
                                                            }
                            ...
                            <div class="table-responsive">
                                <table class="table email-table no-wrap table-hover v-middle mb-0 font-14">
                                    <tbody>
                                        ...
                                        <!-- row -->
                                        <?php
                                          $det=mysqli_query($con,"select * from sms_leave where status='In Review' order by id desc");
                                          $i="1";
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a class="nav-link" href="#" data-toggle="collapse" aria-expanded="false" data-target="#<?php echo $i; ?>" aria-controls="<?php echo $i; ?>">
                                                    <span class="text-dark"><?php echo $p['reason']; ?></span>
                                                </a>
                                                <div id="<?php echo $i; ?>" class="collapse submenu">
                                                    <pre style="font-family: Franklin Gothic; font-size: 15px;"><?php echo $p['description']; ?><br><br>From :&nbsp;<?php echo $p['d-from']; ?>       To :&nbsp;<?php echo $p['d-to']; ?>
                                                    </pre>
                                                    <form action="#" method="post">
                                                        <button type="submit" name="approve" value="<?php echo $p['id']; ?>"><i class="fas fa-pencil-alt">&nbsp;Approve</i></button>
                                                        <button type="submit" name="decline" value="<?php echo $p['id']; ?>"><i class="fas fa-pencil-alt">&nbsp;Decline</i></button>
                                                    </form>
                                                </div>
                                            </td>
                                            <!-- Time -->
                                            <td class="text-muted"><?php echo $p['date']; ?></td>
                                        </tr>
                                        <?php
                                            $i++;
                                          }
